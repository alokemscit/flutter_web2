...
import 'package:web_2/deltaConverter.dart';

// void main() async {
//   WidgetsFlutterBinding.ensureInitialized();
//   runApp(const MaterialApp(debugShowCheckedModeBanner: false, home: Testing6()));
// }

class Testing6 extends StatefulWidget {
  const Testing6({super.key});

  @override
  State<Testing6> createState() => _Testing6State();
}

class _Testing6State extends State<Testing6> {
    ...
    return SafeArea(
        ...
        body: Padding(
          ...
          child: Column(
            children: [
              ...
              ElevatedButton(
                  onPressed: () async {
                    Map<dynamic, dynamic> l = await controller.getDelta();
                    var jcode = jsonEncode(l);
                    var x1 = jsonDecode(jcode);
                    print(x1['ops']);
                    var y = await Delta.fromJson(x1['ops']);
                    // print(y);
                    var k = await y.toHtml();
                   // print(k);
      ...
      onLayout: (format) async => await Printing.convertHtml(
        ...
        html: k1,
                    ...
                    // var x = await controller.getDelta();
                    // var json = jsonEncode(controller.document.toDelta().toJson());

                    //print(x);
                    //  var x1 = jsonDecode(x.toString());

                    // var y = Delta.fromJson(x1);
